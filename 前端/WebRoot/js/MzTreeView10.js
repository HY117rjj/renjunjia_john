...
MzTreeView.prototype.nodeInit = function(sourceIndex, parentId)
{
  ...
  var ctrlId = this.getAttribute(source, "ctrlId");
  var ctrlType = this.getAttribute(source, "ctrlType");        //由xuStanly_cn增加于2006.8.4
  var ctrlValue = this.getAttribute(source, "ctrlValue");	     //由Lancelot增加于2007.5.8
  ...
  this.node[this.index] =
  {
    ...
    "ctrlValue" : ctrlValue      //由Lancelot增加于2007.5.8
  };
...
MzTreeView.prototype.nodeToHTML = function(node, AtEnd)
{
  ...
  if(node.ctrl) HTML += "<input type='"+ node.ctrlType +"' name='"+ node.ctrlName +"' id='"+ node.ctrlId +"' onclick='"+ node.ctrlClick +"' value='"+ node.ctrlValue +"'>"        //由xuStanly_cn增加于2006.5.4
  HTML += "<A "+
...
//    if(k==9 && (new Date().getTime()-d)>20)  屏蔽掉这两句代码，就可以让树始终用图片节点
