  ...
  <head>
	...
	<script type="text/javascript">
		function delStudent(id){
			var url = "${pageContext.request.contextPath }/student/removeStudent.do?ajax=1&id="+id;
			$.get(url, function(json){
				if(json.statusCode==200){
					window.location.href="${pageContext.request.contextPath }/student/list.do.do";
				}
			},'json');
		}
		
		function addStudent(){
			window.location.href="${pageContext.request.contextPath }/student/editStudent.do";
		}
		
		function editStudent(id){
			window.location.href="${pageContext.request.contextPath }/student/editStudent.do?id="+id;
		}
		
		function seach(){
			var type = $("#type").val();
			var seach = $("#seach").val();
			window.location.href = "${pageContext.request.contextPath }/student/list.do?name="+seach;
		}
	</script>
  </head>
<body>
      ...
      <div class="am-g">
        <div class="am-u-sm-12 am-u-md-6">
          <div class="am-btn-toolbar">
            ...
            <!-- 
             ...
             -->
             <a class="am-btn am-btn-default" href="${pageContext.request.contextPath }/student/editStudent.do"><span class="am-icon-plus"></span> 新增</a>
            </div>
          </div>
        </div>
        ...
        <div class="am-u-sm-12 am-u-md-3">
          <div class="am-input-group am-input-group-sm">
            <input type="text" id="seach" value="${name }" class="am-form-field">
          <span class="am-input-group-btn">
            <button class="am-btn am-btn-default" onclick="seach();" type="button">搜索</button>
          </span>
          </div>
        </div>
      </div>
          ...
          <form class="am-form">
            <table class="am-table am-table-striped am-table-hover table-main">
              ...
              <tr>
                ...
                <th class="table-type am-hide-sm-only">身份证号</th>
                <th class="table-author">姓名</th>
                <th class="table-title am-hide-sm-only">学号</th>
              ...
              <c:forEach items="${pd.rows }" var="pd">
              	<tr>
	            	<td>${pd.id }</td>
	                <td class="am-hide-sm-only">${pd.idcard }</td>
	                <td>${pd.name }</td>
	                <td class="am-hide-sm-only">${pd.userName }</td>
	                <td>
	                  <div class="am-btn-toolbar">
	                    <div class="am-btn-group am-btn-group-xs">
	                      <a class="am-btn am-btn-default am-btn-xs am-text-secondary" href="${pageContext.request.contextPath }/student/editStudent.do?id=${pd.id }"><span class="am-icon-pencil-square-o"></span> 编辑</a>
	                      ...
	                      <button class="am-btn am-btn-default am-btn-xs am-text-danger am-hide-sm-only" onclick="delStudent('${pd.id }');"><span class="am-icon-trash-o"></span> 删除</button>
	                    </div>
	                  </div>
	                </td>
	          	</tr>
            ...
            <div class="am-cf">
              	共 ${pd.total } 条记录
              	<div id="page" data-am-page="{pages:'${pd.totalPage }',jump:'./student/list.do?name=${name }&page=%page%'}"></div>
            </div>
          </form>
        </div>
      </div>
