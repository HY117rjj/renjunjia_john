...
<html class="no-js fixed-layout">
  <head>
	...
	<script type="text/javascript">
		function delLog(id){
			var url = "${pageContext.request.contextPath }/process/removeLog.do?ajax=1&id="+id;
			$.get(url, function(json){
				if(json.statusCode==200){
					window.location.href="${pageContext.request.contextPath }/process/list.do";
				}
			},'json');
		}
		
		function addLog(){
			window.location.href="${pageContext.request.contextPath }/process/editLog.do";
		}
		
		function editLog(id){
			window.location.href="${pageContext.request.contextPath }/process/editLog.do?id="+id;
		}
        ...
        <div class="am-u-sm-12 am-u-md-6">
          <div class="am-btn-toolbar">
            ...
            <!-- 
             ...
             -->
             <a class="am-btn am-btn-default" href="${pageContext.request.contextPath }/process/editProcess.do"><span class="am-icon-plus"></span> 新增</a>
            </div>
          </div>
        </div>
          ...
          <form class="am-form">
            <table class="am-table am-table-striped am-table-hover table-main">
              ...
              <tr>
                ...
                <th class="table-title">系统管理员姓名</th>
                <th class="table-type am-hide-sm-only" style="overflow:hidden;white-space:nowrap;text-overflow:ellipsis;">申请内容</th>
                <th class="table-title">安全保密管理员姓名</th>
                <th class="table-title">安全保密管理员审批结论</th>
                <th class="table-author am-hide-sm-only">记录时间</th>
              ...
              <c:forEach items="${pd.rows }" var="pd">
              	<tr>
	            	<td>${pd.id }</td>
	                <td><a href="function:void(0);">${pd.systemName }</a></td>
	                <td class="am-hide-sm-only">${pd.content }</td>
	                <td><a href="function:void(0);">${pd.securityName }</a></td>
	                <td>${pd.conclusion }</td>
                	...
                	<td>
	                  <div class="am-btn-toolbar">
		                <div class="am-btn-group am-btn-group-xs">
		                  <a class="am-btn am-btn-default am-btn-xs am-text-secondary" href="${pageContext.request.contextPath }/process/editProcess.do?id=${pd.id }"><span class="am-icon-pencil-square-o"></span> 审核</a>
		                </div>
	                  </div>
	                </td>
                	</c:if>
            ...
            <div class="am-cf">
              	共 ${pd.total } 条记录
              	<div id="page" data-am-page="{pages:'${pd.totalPage }',jump:'./process/list.do?page=%page%'}"></div>
            </div>
          </form>
        </div>
      </div>
