  ...
  <head>
	...
	<script type="text/javascript">
		function delProject(id){
			var url = "${pageContext.request.contextPath }/project/removeProject.do?ajax=1&id="+id;
			$.get(url, function(json){
				if(json.statusCode==200){
					window.location.href="${pageContext.request.contextPath }/project/list.do";
				}
			},'json');
		}
		
		function addProject(){
			window.location.href="${pageContext.request.contextPath }/project/editProject.do";
		}
		
		function editProject(id){
			window.location.href="${pageContext.request.contextPath }/project/editProject.do?id="+id;
		}
	</script>
  </head>
<body>
        ...
        <div class="am-u-sm-12 am-u-md-6">
          <div class="am-btn-toolbar">
            ...
            <!-- 
             ...
             -->
             <a class="am-btn am-btn-default" href="${pageContext.request.contextPath }/project/editProject.do"><span class="am-icon-plus"></span> 新增</a>
            </div>
          </div>
        </div>
          ...
          <form class="am-form">
            <table class="am-table am-table-striped am-table-hover table-main">
              ...
              <tr>
                ...
                <th class="table-author am-hide-sm-only">内容提要</th>
                <th class="table-date am-hide-sm-only">学分</th>
              ...
              <c:forEach items="${pd.rows }" var="pd">
              	<tr>
	            	<td>${pd.id }</td>
	                <td class="am-hide-sm-only"><a href="function:void(0);">${pd.eduId }</a></td>
	                <td>${pd.eduDesignation }</td>
	                <td class="am-hide-sm-only">${pd.summary }</td>
	                <td class="am-hide-sm-only">${pd.creditHour }</td>
	                <td>
	                  <div class="am-btn-toolbar">
	                    <div class="am-btn-group am-btn-group-xs">
	                      <a class="am-btn am-btn-default am-btn-xs am-text-secondary" href="${pageContext.request.contextPath }/project/editProject.do?id=${pd.id }"><span class="am-icon-pencil-square-o"></span> 编辑</a>
	                      ...
	                      <button class="am-btn am-btn-default am-btn-xs am-text-danger am-hide-sm-only" onclick="delProject('${pd.id }');"><span class="am-icon-trash-o"></span> 删除</button>
	                    </div>
	                  </div>
	                </td>
	          	</tr>
            ...
            <div class="am-cf">
              	共 ${pd.total } 条记录
              	<div id="page" data-am-page="{pages:'${pd.totalPage }',jump:'./project/list.do?page=%page%'}"></div>
            </div>
          </form>
        </div>
      </div>
