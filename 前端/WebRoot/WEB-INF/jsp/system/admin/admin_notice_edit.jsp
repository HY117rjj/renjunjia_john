  ...
  <head>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			CKEDITOR.replace('newsArea', {
				filebrowserImageUploadUrl: './uploadNewsImg.do?basePath='+basePath
			});
			CKEDITOR.tools.addFunction(function(text) {
				var dialog = window.CKEDITOR.dialog.getCurrent();
				$("#"+ dialog.getContentElement('info', 'txtUrl').getInputElement().$.id).val(text);
...
<div class="am-cf admin-main">
  ...
  <div class="admin-content">
    <div class="admin-content-body">
      ...
      <hr>
		<form method="post" class="am-form" action="./notice/saveNotice.do">
			<input type="hidden" name="id" value="${notice.id }"/>
			<fieldset>
    		<legend>编辑通知信息</legend>
    		<div class="am-form-group">
		      <label for="title">通知标题</label>
		      <input type="text" class="am-form-field" id="title" name="title" value="${notice.title }" placeholder="" required>
		    </div>
		    <div class="am-form-group">
		      <label for="content">通知内容</label>
		      <textarea id="newsArea" rows="10" cols="70" name="content">${notice.content }</textarea>
		    </div>
		    <div class="am-form-group">
		      <label for="teachingTime_">授课时间</label>
		      <div class="am-form-group am-form-icon">
                  <i class="am-icon-calendar"></i>
                  <input type="datetime-local" class="am-form-field am-input-sm" id="teachingTime_" name="teachingTime_" value="${notice.teachingTime_ }" placeholder="日期">
              </div>
		    </div>
    		</fieldset>
