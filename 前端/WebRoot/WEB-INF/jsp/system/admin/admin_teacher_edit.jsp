...
<div class="am-cf admin-main">
  ...
  <div class="admin-content">
    <div class="admin-content-body">
      ...
      <hr>
		<form method="post" class="am-form" action="./teacher/saveTeacher.do">
			<input type="hidden" name="id" value="${teacher.id }"/>
			<fieldset>
    		<legend>编辑授课人员信息</legend>
    		<div class="am-form-group">
		      <label for="idcard">教师身份证号码</label>
		      <input type="text" class="am-form-field" id="idcard" name="idcard" value="${teacher.idcard }" placeholder="输入身份证号码" required>
		    </div>
		    <div class="am-form-group">
		      <label for="name">教师姓名</label>
		      <input type="text" class="am-form-field" id="name" name="name" value="${teacher.name }" placeholder="输入姓名" required>
		    </div>
		    <div class="am-form-group">
		      <label for="subject">专业学科</label>
		      <select id="subject" name="subject">
		      	<c:forEach items="${zyxk }" var="zz">
		      		<option value="${zz.code }" ${teacher.subject == zz.code ? 'selected="selected"' : '' }>${zz.name }</option>
		      	</c:forEach>
		      </select>
    		...
    		<div class="am-form-group">
		      ...
		      <select multiple class="" id="major" name="major">
		        <c:forEach items="${zlkm }" var="zz">
		      		<option value="${zz.code }" ${teacher.major == zz.code ? 'selected="selected"' : '' }>${zz.name }</option>
		      	</c:forEach>
		      </select>
		    </div>
    		<div class="am-form-group">
		      <label for="basicCharge">基础授课费</label>
		      <input type="text" class="am-form-field" id="basicCharge" name="basicCharge" value="${teacher.basicCharge }" placeholder="">
		    </div>
		    <div class="am-form-group">
		      <label for="research">授课及研究方向</label>
		      <textarea class="" rows="5" id="research" name="research">${teacher.research }</textarea>
		    </div>
		    <div class="am-form-group">
		      <label for="teachingSign">是否授课</label><br/>
		      <label class="am-radio-inline">
		        <input type="radio" value="1" name="teachingSign" ${teacher.teachingSign eq '1' ? 'checked="checked"' : ''}> 授课
		      </label>
		      <label class="am-radio-inline">
		        <input type="radio" value="0" name="teachingSign" ${teacher.teachingSign eq '0' ? 'checked="checked"' : ''}> 停止授课
		      </label>
		    </div>
    		</fieldset>
