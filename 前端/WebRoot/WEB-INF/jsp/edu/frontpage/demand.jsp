...
<head>
	...
	function demandCourseware(id, charge){
		// 预判断账户余额与收费比较 待完善
		if(charge = "" || charge <= 0){
			window.open("${pageContext.request.contextPath }/demand/demandCourseware.do?id="+id);
		}else{
			$('#my-confirm').modal({
	        	relatedTarget: this,
	  ...
	  <div class="am-g">
        <div class="am-u-sm-12">
          <form class="am-form">
            ...
            <div class="am-cf">
              	共 ${pd.total } 条记录
              	<div id="page" data-am-page="{pages:'${pd.totalPage }',jump:'./demand/list.do?type=${type }&code=${code }&page=%page%'}"></div>
            </div>
          </form>
        </div>
      </div>	     
