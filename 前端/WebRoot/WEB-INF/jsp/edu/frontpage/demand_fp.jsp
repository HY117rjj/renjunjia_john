...
<head>
	...
	function demandCourseware(id, charge){
		// 预判断账户余额与收费比较 待完善
		if(charge = "" || charge <= 0){
			window.open("${pageContext.request.contextPath }/demand/demandCoursewareForFrontPage.do?id="+id);
		}else{
			$('#my-confirm').modal({
	        	relatedTarget: this,
            ...
            <ul class="am-nav am-nav-pills am-topbar-nav">
                ...
                <li class="am-dropdown" data-am-dropdown>
                    ...
                    <ul class="am-dropdown-content">
                        ...
                        <li><a href="./demand/listForFrontPage.do?type=1">1. 专业学科</a></li>
                        <li><a href="./demand/listForFrontPage.do?type=2">2. 临床专业</a></li>
                        <li><a href="./demand/listForFrontPage.do?type=3">3. 疾病分类</a></li>
                        <li><a href="./demand/listForFrontPage.do?type=4">4. 讲授教师</a></li>
                    </ul>
                </li>
                <li class="am-dropdown" data-am-dropdown>
                    <a class="am-dropdown-toggle" data-am-dropdown-toggle href="javascript:;">
    ...
    <div class="am-offcanvas-bar admin-offcanvas-bar">
      <ul class="am-list admin-sidebar-list" id="collapase-nav-1">
        ...
        <c:forEach items="${dicdata }" var="cc">
        	...
        	<c:if test="${type > 2 }">
        	  <a href="./demand/listForFrontPage.do?type=${type }&code=${cc.code }"><span class="am-icon-calendar"></span> ${cc.name }</a>
        	</c:if>
        	<c:if test="${type <= 2 }">
	          ...
	          <ul class="am-list am-collapse admin-sidebar-sub am-in" id="collapse-nav${cc.code }">
	          	<c:forEach items="${cc.dataList }" var="cc2">
	          		<c:if test="${empty cc2.dataList }">
        				<li><a href="./demand/listForFrontPage.do?type=${type }&code=${cc2.code }"><span class="am-icon-calendar"></span> ${cc2.name }</a></li>
        	  		</c:if>
        	  		<c:if test="${not empty cc2.dataList }">
        				<li><a data-am-collapse="{target: '#collapse-nav${cc2.name }'}"><span class="am-icon-calendar"></span> ${cc2.name }<span class="am-icon-angle-right am-fr am-margin-right"></span></a></li>
        	  			...
        	  			<c:forEach items="${cc2.dataList }" var="cc3">
			        		<li><a href="./demand/listForFrontPage.do?type=${type }&code=${cc3.code }"><span class="am-icon-calendar"></span> ${cc3.name }</a></li>
			          	</c:forEach>
			          	</ul>
        	  		</c:if>
	          	</c:forEach>
  ...
  <div class="admin-content">
    <div class="admin-content-body">
      <div class="am-cf am-padding am-padding-bottom-0">
        <div class="am-fl am-cf"><strong class="am-text-primary am-text-lg">课件点播</strong> / <small>${typeName }</small></div>
      </div>
          ...
          <form class="am-form">
            <table class="am-table am-table-striped am-table-hover table-main" >
              ...
              <tr>
                ...
                <th class="table-date">课程题目</th>
                <th class="table-date am-hide-sm-only">点播费</th>
              ...
              <c:forEach items="${pd.rows }" var="pd">
              	<tr>
	            	<td class="am-hide-sm-only">${pd.id }</td>
	                ...
	                <td>${pd.title }</td>
	                <td class="am-hide-sm-only">${pd.onDemandCharge }</td>
	                <td>
	                  <div class="am-btn-toolbar">
	                    <div class="am-btn-group am-btn-group-xs">
	                      <a class="am-btn am-btn-default am-btn-xs am-text-secondary" href="javascript:void(0);" onclick="demandCourseware('${pd.id }','${pd.onDemandCharge }');" target="_blank"><span class="am-icon-pencil-square-o"></span> 点播</a>
	                    </div>
	                  </div>
	                </td>
	          	</tr>
            ...
            <div class="am-cf">
              	共 ${pd.total } 条记录
              	<div id="page" data-am-page="{pages:'${pd.totalPage }',jump:'./demand/listForFrontPage.do?type=${type }&code=${code }&page=%page%'}"></div>
            </div>
          </form>
        </div>
      </div>	     
