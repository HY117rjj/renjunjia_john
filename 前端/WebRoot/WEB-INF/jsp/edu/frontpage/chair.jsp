    ...
    <div class="am-offcanvas-bar admin-offcanvas-bar">
      <ul class="am-list admin-sidebar-list" id="collapase-nav-1">
        ...
        <c:forEach items="${dicdata }" var="cc">
        	...
        	<c:if test="${type > 2 }">
        	  <a href="./chair/list.do?type=${type }&code=${cc.code }"><span class="am-icon-calendar"></span> ${cc.name }</a>
        	</c:if>
        	<c:if test="${type <= 2 }">
	          ...
	          <ul class="am-list am-collapse admin-sidebar-sub am-in" id="collapse-nav${cc.code }">
	          	<c:forEach items="${cc.dataList }" var="cc2">
	          		<c:if test="${empty cc2.dataList }">
        				<li><a href="./chair/list.do?type=${type }&code=${cc2.code }"><span class="am-icon-calendar"></span> ${cc2.name }</a></li>
        	  		</c:if>
        	  		<c:if test="${not empty cc2.dataList }">
        				<li><a data-am-collapse="{target: '#collapse-nav${cc2.name }'}"><span class="am-icon-calendar"></span> ${cc2.name }<span class="am-icon-angle-right am-fr am-margin-right"></span></a></li>
        	  			...
        	  			<c:forEach items="${cc2.dataList }" var="cc3">
			        		<li><a href="./chair/list.do?type=${type }&code=${cc3.code }"><span class="am-icon-calendar"></span> ${cc3.name }</a></li>
			          	</c:forEach>
			          	</ul>
        	  		</c:if>
	          	</c:forEach>
        ...
        <li><a href="./chair/listAppointment.do?type=${type }"><span class="am-icon-pencil-square-o"></span> 预约记录</a></li>
        <li><a href="./chair/listCourse.do?type=${type }"><span class="am-icon-pencil-square-o"></span> 我的课程</a></li>
  ...
  <div class="admin-content">
    <div class="admin-content-body">
      <div class="am-cf am-padding am-padding-bottom-0">
        <div class="am-fl am-cf"><strong class="am-text-primary am-text-lg">预约讲座</strong> / <small>${typeName }</small></div>
      </div>
	  ...
	  <div class="am-g">
        <div class="am-u-sm-12">
          <!-- delete form or add onsubmit="return false;" incase button submit -->
          <form class="am-form" action="" onsubmit="return false;">
              ...
              <c:forEach items="${pd.rows }" var="pd">
              	<tr>
	                ...
	                <td>
	                  <div class="am-btn-toolbar">
	                    <div class="am-btn-group am-btn-group-xs">
	                      <!-- <a class="am-btn am-btn-default am-btn-xs am-text-secondary" href="${pageContext.request.contextPath }/chair/chairLecture.do?id=${pd.id }"><span class="am-icon-pencil-square-o"></span> 预约</a>-->
	                      <button class="am-btn am-btn-default am-btn-xs am-text-danger am-hide-sm-only" onclick="chairLecture('${pd.id }');"><span class="am-icon-trash-o"></span> 预约</button>
	                    </div>
	                  </div>
	                </td>
	          	</tr>
            ...
            <div class="am-cf">
              	共 ${pd.total } 条记录
              	<div id="page" data-am-page="{pages:'${pd.totalPage }',jump:'./chair/list.do?type=${type }&code=${code }&page=%page%'}"></div>
            </div>
          </form>
        </div>
      </div>	     
