    ...
    <div class="am-container">
        ...
        <div class="am-collapse am-topbar-collapse" id="collapse-head">
            <ul class="am-nav am-nav-pills am-topbar-nav">
                ...
                <li class="am-active"><a href="./demand/listHE.do?type=3">健康教育</a></li>
    ...
    <div class="am-offcanvas-bar admin-offcanvas-bar">
      <ul class="am-list admin-sidebar-list" id="collapase-nav-1">
        ...
        <c:forEach items="${dicdata }" var="cc">
        	...
        	<c:if test="${type > 2 }">
        	  <a href="./demand/listHE.do?type=${type }&code=${cc.code }"><span class="am-icon-calendar"></span> ${cc.name }</a>
        	</c:if>
        	<c:if test="${type <= 2 }">
	          ...
	          <ul class="am-list am-collapse admin-sidebar-sub am-in" id="collapse-nav${cc.code }">
	          	<c:forEach items="${cc.dataList }" var="cc2">
	          		<c:if test="${empty cc2.dataList }">
        				<li><a href="./demand/listHE.do?type=${type }&code=${cc2.code }"><span class="am-icon-calendar"></span> ${cc2.name }</a></li>
        	  		</c:if>
        	  		<c:if test="${not empty cc2.dataList }">
        				<li><a data-am-collapse="{target: '#collapse-nav${cc2.name }'}"><span class="am-icon-calendar"></span> ${cc2.name }<span class="am-icon-angle-right am-fr am-margin-right"></span></a></li>
        	  			...
        	  			<c:forEach items="${cc2.dataList }" var="cc3">
			        		<li><a href="./demand/listHE.do?type=${type }&code=${cc3.code }"><span class="am-icon-calendar"></span> ${cc3.name }</a></li>
			          	</c:forEach>
			          	</ul>
        	  		</c:if>
	          	</c:forEach>
	  ...
	  <div class="am-g">
        <div class="am-u-sm-12">
          <form class="am-form">
              ...
              <c:forEach items="${pd.rows }" var="pd">
              	<tr>
	                ...
	                <td>
	                  <div class="am-btn-toolbar">
	                    <div class="am-btn-group am-btn-group-xs">
	                      <a class="am-btn am-btn-default am-btn-xs am-text-secondary" href="${pageContext.request.contextPath }/demand/demandCourseware.do?id=${pd.id }"><span class="am-icon-pencil-square-o"></span> 点播</a>
	                    </div>
	                  </div>
	                </td>
	          	</tr>
            ...
            <div class="am-cf">
              	共 ${pd.total } 条记录
              	<div id="page" data-am-page="{pages:'${pd.totalPage }',jump:'./demand/listHE.do?type=${type }&code=${code }&page=%page%'}"></div>
            </div>
          </form>
        </div>
      </div>	     
